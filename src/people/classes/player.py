...
class Player(Person):
    ...
    def convert_child_to_player(
        self, c
    ):  # This is here for when it's possible to continue as your child
    ...
    def process_relation_death(self, relation, reason=None):
        ...
        print(
            _("Your {relative} died at the age of {age}.\n{he_she} {reason}.").format(
                relative=rel_str, age=relation.age, he_she=he_she, reason=reason
            )
        )
        inheritance = 0
        ...
        if self.gender == Gender.Female:
            if self.is_pregnant:
                ...
                if not x_in_y(self.fertility, 100) and one_in(5):
                    ...
                    print(
                        _("You just had a baby {son_daughter}!").format(
                            son_daughter=typ
                        )
                    )
                    him_her = c.him_her()
            ...
            if not x_in_y(self.partner.fertility, 100) and one_in(5):
                display_event(
                    _(
                        "Your {partner} suffered a miscarriage and lost the baby!"
                    ).format(partner=self.partner.name_accusative())
                )
                self.change_happiness(-randint(30, 50))
            else:
                print(
                ...
                print(
                    _(
                        "You are the father of a baby {son_daughter} named {name}."
                    ).format(son_daughter=typ, name=c.name)
                )
        ...
        for relation in self.relations[:]:
            ...
            if sudden_death or relation.death_check():
                ...
                if sudden_death:
                    ...
                    if relation.age >= 16:
                        sudden_death_reasons.append(
                            _(
                                "died after driving off a cliff while texting and driving"
                            )
                        )
                    reason = random.choice(sudden_death_reasons)
                    if one_in(1000):
                        reason = _(
                            "starved to death after playing this game for too many days without stopping to eat"
                        )
        ...
        if self.has_job:
            ...
            if self.can_retire() and one_in(3):
                ...
                if self.age >= 70 and one_in(3):
                    display_event(
                        _("You have been forced to retire from your position.")
                    )
                    self.lose_job()
                    print(
                        _(
                            "You are now receiving a yearly pension of ${pension:,}."
                    ...
                    print(
                        _(
                            "You are eligible to retire and begin receiving a yearly pension of ${pension:,}."
                        ).format(pension=pension)
                    )
        ...
        if 1 <= self.marital_status <= 2:
            ...
            if self.partner.relationship <= randint(15, 30) and one_in(6):
                ...
                if choice == 1:
                    ...
                    he_she = self.partner.he_she().capitalize()
                    if x_in_y(self.partner.willpower, 100) or x_in_y(
                        self.partner.willpower, 100
                    ):
                        display_event(
